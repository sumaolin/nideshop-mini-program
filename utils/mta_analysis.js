...
var MTA = {
  App: {
    init: function(a) {
      ...
      setSID()
      'lauchOpts' in a &&
        ((MTA.Data.lanchInfo = a.lauchOpts), (MTA.Data.lanchInfo.landing = 1))
    }
  },
  Page: {
    init: function() {
      ...
      if ('' != MTA_CONFIG.app_id) {
        ...
        MTA.Data.lanchInfo &&
          ...
          1 == MTA.Data.lanchInfo.landing &&
            ((b.ext += ';lp=1'), (MTA.Data.lanchInfo.landing = 0)))
        c.push({
          rand: +new Date()
        })
        b = 0
        for (var d = c.length; b < d; b++)
          for (var e in c[b])
            c[b].hasOwnProperty(e) &&
              a.push(e + '=' + ('undefined' == typeof c[b][e] ? '' : c[b][e]))
        wx.request({
    ...
    stat: function(a, b) {
      if ('' != MTA_CONFIG.event_id) {
        ...
        for (e = d.length; f < e; f++)
          for (var h in d[f])
            d[f].hasOwnProperty(h) &&
              c.push(h + '=' + ('undefined' == typeof d[f][h] ? '' : d[f][h]))
        wx.request({
